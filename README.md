...
I didn't want people to supply their username and passwords so I decided to resort to using cookies. I understand that it isn't better or more secure but at-least only those who actually knows what their doing can use the script and not someone unexperienced to avoid problems.

Usage Limits
------------
Facebook spam blocks you for a few minutes when doing a lot of requests even via the frontend so use this only when needed.
...
- On your browser go to `http://localhost:8080/usernameOrID`, example: [http://localhost:8080/madziikoy](http://localhost:8080/madziikoy) and wait for awhile and it should return the year the account was created. 
